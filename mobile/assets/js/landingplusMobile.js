...
document.addEventListener("DOMContentLoaded", function () {
  ...
  openPopGallery.forEach((element) => {
    ...
    let popId = element.getAttribute("data-popid");
      console.log("popId", popId);

      fetch(`/load-popGallery.inc?id=${popId}&refresh=true`)
        .then((response) => response.text())

        .then((userData) => {
            galleryPop.innerHTML = "";
            showPopupGallry.style.display = "flex";            
          galleryPop.innerHTML = "";
          // console.log(projectListContainer);
          console.log("userData", userData);
            galleryPop.innerHTML = userData;
            popToolsFn();
        })        
        .catch((error) => {
  ...
  function popToolsFn(params) {
    ...
    //   innerGallery.classList.remove("-translate-y-[100vh]");
    // });
    // let galleryPopBoxes = document.querySelectorAll(
    //   ".galleryPop .innerGallery .boxImg"
    // );
    galleryPopBoxes.forEach((element, i) => {
      element.addEventListener("click", function (params) {
    ...
    zoomIn.addEventListener("click", function (params) {
      ...
      let activeSlide = document.querySelector(
        ".galleryPopSlider .swiper-slide-active img"
      );
